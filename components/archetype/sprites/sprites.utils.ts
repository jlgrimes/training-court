const customSpritePaths: Record<string, string> = {
  'crushing-hammer': '/assets/sprites/crushing-hammer.png',
};

const customSpriteUrls = Object.values(customSpritePaths);
...
export const fetchLimitlessSprites = async (): Promise<string[]> => {
  try {
    ...
    if (json['code'] !== 200) return customSpriteUrls;
    return [...(json['pokedex'] as string[]), ...customSpriteUrls];
  } catch {
    return customSpriteUrls;
  }
};
...
export const pkmnToImgSrc = (pkmn: string | null | undefined) => {
  if (!pkmn) return undefined;

  const normalizedName = pkmn.toLowerCase().trim().replace(/\s+/g, '-');
  return customSpritePaths[normalizedName] ?? `https://r2.limitlesstcg.net/pokemon/gen9/${normalizedName}.png`;
};
