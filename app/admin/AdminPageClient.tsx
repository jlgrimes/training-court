...
import { countAllUsers, countUsersInLastXDays, fetchAllFeedback, fetchCommonlyUsedAvatars, calculatePercentageChange, countAllLogs } from "@/components/admin/admin.data.utils";
...
export function AdminPageClient({ allAvatarImages }: { allAvatarImages: string[] }) {
  ...
  const isAdmin = !!user && isUserAnAdmin(user.id);
  ...
  const lastSevenDaysUsers = lastSevenDaysUsersRaw ?? 0;
  const previousSevenDaysUsers = previousSevenDaysUsersRaw ?? 0;
